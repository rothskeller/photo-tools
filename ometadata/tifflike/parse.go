...
// NewTIFFLike returns a handler for a TIFF-like file (or portion of file)
// covered by the specified reader.  It returns an error if the data read from
// the reader does not conform to TIFF layout.
func NewTIFFLike(r tiffReader) (t *TIFFLike, err error) {
	var (
		buf [8]byte
	)
	if n, _ := r.ReadAt(buf[:8], 0); n < 4 {
		return nil, errors.New("invalid TIFF header")
	}
	t = &TIFFLike{r: r}
...
func (t *TIFFLike) Encoding() binary.ByteOrder { return t.enc }
